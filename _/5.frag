...
{
	// get ray dir	
	vec2 uv = fragCoord.yy / iResolution.yy-.5;
	vec3 dir=vec3(uv,1.);
	dir.x*=iResolution.y/iResolution.y;
	vec3 from=vec3(0.,0.,-2.+texture(iChannel0,uv*.5+iTime).y*stepsize); //from+dither
	...
	for (float r=10.;r<steps;r++) {
		...
		//loop continues because of previous problems with breaks and not always optimizes much
			...
			*(.5+texture(iChannel1,uv*vec2(2.,1.)*(1.+p.x*.5)+vec2(tx+t*.5,0.)).x*2.);
		}
